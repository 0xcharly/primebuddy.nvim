...
Color.new('statusLineBg', theme.canvas.overlay)
...
-- Syntax.
Color.new('syntaxBooleanLiteral', scale.blue.shade2)
Color.new('syntaxComment', scale.gray.shade4)
Color.new('syntaxDeprecated', scale.gray.shade3)
Color.new('syntaxConstant', scale.orange.shade2)
Color.new('syntaxFunction', scale.purple.shade2)
Color.new('syntaxFloatLiteral', scale.red.shade3)
Color.new('syntaxEntity', theme.fg.default)
Color.new('syntaxEntityTag', scale.green.shade1)
Color.new('syntaxField', scale.blue.shade2)
...
Group.new('Search', c.fg, c.hl) -- Last search pattern highlighting (see 'hlsearch').  Also used for similar items that need to stand out.
...
Group.new('StatusLine', c.fg, c.statusLineBg) -- status line of current window
Group.new('StatusLineNC', c.fgOnCanvasMuted, c.statusLineBg) -- status lines of not-current windows Note: if this is equal to "StatusLine" Vim will use "^^^" in the status line of the current window.
...
Group.new('Comment', c.syntaxComment) -- any comment
...
Group.new('Boolean', c.syntaxBooleanLiteral) -- a boolean constant: TRUE, false
Group.new('Float', c.syntaxFloatLiteral) -- a floating point constant: 2.3e10
...
Group.new('Operator', c.syntaxPunctuation) -- "sizeof", "+", "*", etc.
Group.new('Keyword', c.syntaxKeyword) --  any other keyword
...
Group.new('SpecialChar', c.syntaxSpecialChar) -- special character in a constant
-- Group.link('Tag', g.Special) -- you can use CTRL-] on this
Group.new('Delimiter', c.syntaxPunctuation) -- character that needs attention
...
Group.new('LspDiagnosticsVirtualTextWarning', c.roleAttention, c.roleAttentionBg) -- Used for "Warning" diagnostic virtual text
...
Group.new('LspDiagnosticsUnderlineWarning', nil, nil, s.undercurl, c.roleAttention) -- Used to underline "Warning" diagnostics
Group.new('LspDiagnosticsUnderlineInformation', nil, nil, s.undercurl, c.roleAccent) -- Used to underline "Information" diagnostics
...
Group.new('TSBoolean', c.syntaxBooleanLiteral) -- Boolean literals: `True` and `False` in Python.
Group.new('TSCharacter', c.syntaxLiteral) -- Character literals: `'a'` in C.
Group.new('TSComment', c.syntaxComment) -- Line comments and block comments.
...
Group.new('TSField', c.syntaxField) -- Object and struct fields.
Group.new('@field', c.syntaxField) -- Object and struct fields.
...
Group.new('TSProperty', c.syntaxField) -- Same as `TSField`.
